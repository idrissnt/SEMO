...
class CartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    handlePath.moveTo(width * 0.10, height * 0.10);

    // Curve down to connect with the basket
    handlePath.quadraticBezierTo(
        width * 0.19, height * 0.1, width * 0.183, height * 0.25);
    ...
    canvas.drawLine(Offset(0, height * 0.10),
        Offset(width * 0.10, height * 0.10), handleLightPaint);

    // Draw the basket (trapezoidal shape)
    final basketPath = Path();

    // Top-left of basket
    basketPath.moveTo(width * 0.18, height * 0.20);

    // Top-right of basket (higher than left to create slant)
    basketPath.lineTo(width * 0.85, height * 0.22);

    // Bottom-right of basket
    basketPath.lineTo(width * 0.75, height * 0.58);

    // Bottom-left of basket
    basketPath.lineTo(width * 0.20, height * 0.62);
    ...
    for (int i = 0; i <= 4; i++) {
      final initStart = width * (0.30 + (i * 0.1));

      final startX = initStart - width * 0.02;
      final endX = initStart;

      canvas.drawLine(Offset(startX, height * 0.30), Offset(endX, height * 0.5),
          slitsPaint);
    }
    ...
    framePath.moveTo(width * 0.20, height * 0.62);

    // Curve down to the back wheel
    framePath.quadraticBezierTo(
        width * 0.12, height * 0.70, width * 0.25, height * 0.75);
